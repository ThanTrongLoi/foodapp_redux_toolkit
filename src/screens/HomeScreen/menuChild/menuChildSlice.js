import { createSlice } from '@reduxjs/toolkit'

data = [
  {
    type: 'Dessert',
    color: '#f7931e',
    data: [
      {
        name: 'Stewed Mushrooms',
        image: require('../../../assets/images/namkho.jpg'),
        price: "$12"
      },
      {
        name: 'Jackfruit Fried',
        image: require("../../../assets/images/mitkho.jpg"),
        price: "$15"
      }
    ]
  },
  {
    type: 'Main course',
    color: '#39b54a',
    data: [
      {
        name: 'Noodles',
        image: require("../../../assets/images/hutieu.jpg"),
        rating: 4,
        price: "$20"
      },
      {
        name: 'Beef',
        image: require("../../../assets/images/cuonlalot.jpg"),
        rating: 2,
        price: "$12"
      },
      {
        name: 'Salad dressing',
        image: require("../../../assets/images/cuondiep.jpg"),
        rating: 5,
        price: "$13"
      },
    ]
  },
  {
    type: 'Other',
    color: '#ed1e79',
    data: [
      {
        name: 'Salad dressing',
        image: require("../../../assets/images/cuondiep.jpg"),
        price: "$13"
      },
      {
        name: 'Jackfruit warehouse',
        image: require("../../../assets/images/mitkho.jpg"),
        price: "$15"
      }
    ]
  },
]

export const menuChildSlice = createSlice({
  name: 'menuchild',
  initialState: {
    value: data,
  },
  reducers: {
    pending: () => { }
  }
})

export const selectMenuChild = state => state.menuchild.value
export const { pending } = menuChildSlice.actions
export default menuChildSlice.reducer